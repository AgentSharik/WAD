...
Автоматических тестов пока нет — проверка делается прогоном в чистой виртуальной машине.
В репозитории настроены проверки при каждом изменении: разбор синтаксиса всех скриптов под **Windows
PowerShell 5.1** (не только под PowerShell 7) и статический анализ PSScriptAnalyzer на ошибки, плюс
...
- **Дальше:** переработка v2 — один набор модулей вместо двух папок с копиями, честные статусы задач,
  продолжение установки после сбоя или выключения.
