...
T_TEXT_END = 17.55              # приветствие полностью ушло
T_WIN_APPEAR = 18.30            # 0,75 с на экране только фон, потом окно начинает проступать
T_START = 20.30                 # окно проступило целиком (проявляется 2 с, а не «включается»)
T_INSTALL = 26.30               # окно висит ровно 6 с, дальше само начинает установку
T_GITHUB = 49.90                # финал установки держится 1,6 с и окно уходит в проект
T_END = 55.40
...
WELCOME = [
    ...
    (4.75, 5.65, 7.75, 8.85, ['Вас приветствует WAD'], 'wad'),
    (9.10, 10.00, 12.10, 13.20, ['WAD настроит Windows для вас,', 'можете отдохнуть'], 'wad'),
    (13.45, 14.35, 16.45, 17.55, ['Приступаем'], 'go'),
]
